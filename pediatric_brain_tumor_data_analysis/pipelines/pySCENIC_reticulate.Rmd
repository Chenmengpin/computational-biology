...
# TODO
# fivenum on the sets of regulons that are common between the two experimental setups versus those that are not common and specific to each 
# we need to understand what the common and uncommon ones mean/are in terms of identity and activity scores correlation 
...
# dig into the identity of the common/specific regulons
dat_replicate_a_specific_regulons <- as.character(colnames(dat_replicate_a)[!colnames(dat_replicate_a) %in% common_regulons])
dat_ind_time_points_specific_regulons <- as.character(rownames(dat_ind_time_points)[!rownames(dat_ind_time_points) %in% common_regulons])

dat_replicate_a_specific_regulons_dat <- dat_replicate_a[, colnames(dat_replicate_a) %in% dat_replicate_a_specific_regulons]

dat_ind_time_points_specific_regulons_dat <- dat_ind_time_points[rownames(dat_ind_time_points) %in% dat_ind_time_points_specific_regulons,]
# compute correlation
...
df1 <- df1[unique(df1$cells) ,]
df2 <- df2[unique(df2$cells), ]
dat <- inner_join(df1, df2, by = 'regulons')

pdf ('../plots/scatter_plot_repA_vs_ind_time_points.pdf', height = 10, width = 15)
ggplot(dat, aes(x = activity_replicate_A, y = activity_ind_time_points, color = cells.x)) +
  geom_point (shape = 1) + 
...
pheatmap::pheatmap(regulonActivity_byTimePoint_Scaled, fontsize_row=2, fontsize_col = 4,  
                   color=colorRampPalette(c("blue","white","red"))(100), breaks=seq(-3, 3, length.out = 100), annotation= annotation, treeheight_row=10, treeheight_col=10, border_color=NA, clustering_method = 'ward.D2')
dev.off()
```
